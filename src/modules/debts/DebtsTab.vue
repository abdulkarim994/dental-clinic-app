<template>
  <div class="space-y-4">
    ...
    <!-- Empty -->
    <div v-if="!filteredDebts.length" class="text-center py-16 opacity-25">
    ...
    <!-- Debt Cards -->
    <DebtCard v-for="d in filteredDebts" :key="d.id" :debt="d" :currency="cur" :doctor-pct="doctorPct"
...
function delDebt(id) {
  ...
  if (!debt) return
  if (!confirm('حذف سجل الدين نهائياً؟\nالمريض: ' + (debt.name || '—') + '\nسيتم حذف جميع سجلات الدفعات المرتبطة.')) return
