...
const yrRecs = computed(() => app.records.filter(r => r.date?.startsWith(selectedYear.value) && !r.isDebt && !r.isPros && r.payment !== 'دين' && !isProsDebtPay(r, app.debts)))
...
const yearCash = computed(() => sum(yrRecs.value.filter(r => r.payment === 'كاش'), 'amount'))
const yearXfer = computed(() => sum(yrRecs.value.filter(r => r.payment !== 'كاش'), 'amount'))
...
function printAnnual() {
  ...
  const mRows = AR_MONTHS.map((mn, i) => {
    ...
    const mc = sum(mr.filter(r => r.payment === 'كاش'), 'amount')
    const mx = sum(mr.filter(r => r.payment !== 'كاش'), 'amount')
    const { pDoc: md } = prosDocEarnings(mp, mPdPays)
    const mt = mc + mx + md
    return { name: mn, cash: mc, xfer: mx, pros: md, total: mt }
  }).filter(r => r.total > 0)
  ...
  const monthRowsHtml = mRows.map(r => `
    <tr>
      <td style="padding:7px 10px;border-bottom:1px solid #eee;text-align:right;font-weight:600">${r.name}</td>
      <td style="padding:7px 10px;border-bottom:1px solid #eee;text-align:right">${n(r.cash)} ${cur.value}</td>
      <td style="padding:7px 10px;border-bottom:1px solid #eee;text-align:right">${n(r.xfer)} ${cur.value}</td>
      <td style="padding:7px 10px;border-bottom:1px solid #eee;text-align:right">${n(r.pros)} ${cur.value}</td>
      <td style="padding:7px 10px;border-bottom:1px solid #eee;text-align:right;font-weight:800">${n(r.total)} ${cur.value}</td>
    </tr>
  `).join('')
  ...
  printHtml.value = `
    <h1>${clinicName}</h1>
    <p class="sub">الجرد السنوي — ${yr}</p>
    <table>
      <thead><tr>
        <th style="padding:8px 10px;text-align:right">كاش</th>
        <th style="padding:8px 10px;text-align:right">تحويل</th>
        <th style="padding:8px 10px;text-align:right">تركيبات</th>
        <th style="padding:8px 10px;text-align:right">الإجمالي</th>
      </tr></thead>
      <tbody><tr>
        <td style="padding:7px 10px;text-align:right;font-weight:700">${n(grandCash)} ${cur.value}</td>
        <td style="padding:7px 10px;text-align:right;font-weight:700">${n(grandXfer)} ${cur.value}</td>
        <td style="padding:7px 10px;text-align:right;font-weight:700">${n(grandPros)} ${cur.value}</td>
        <td style="padding:7px 10px;text-align:right;font-weight:800;color:#1d4ed8">${n(grandTotal)} ${cur.value}</td>
      </tr></tbody>
    </table>
    ...
    <table>
      <thead><tr>
        ...
        <th style="padding:8px 10px;text-align:right">كاش</th>
        <th style="padding:8px 10px;text-align:right">تحويل</th>
        <th style="padding:8px 10px;text-align:right">تركيبات (${cur.value})</th>
        <th style="padding:8px 10px;text-align:right">الإجمالي</th>
      </tr></thead>
      <tbody>
        ${monthRowsHtml}
        <tr class="total-row">
          <td style="padding:7px 10px;font-weight:800">الإجمالي السنوي</td>
          <td style="padding:7px 10px;font-weight:800">${n(grandCash)} ${cur.value}</td>
          <td style="padding:7px 10px;font-weight:800">${n(grandXfer)} ${cur.value}</td>
          <td style="padding:7px 10px;font-weight:800">${n(grandPros)} ${cur.value}</td>
          <td style="padding:7px 10px;font-weight:900;color:#1d4ed8">${n(grandTotal)} ${cur.value}</td>
        </tr>
      </tbody>
    </table>`
