    ...
    <div v-if="visible" class="modal-ol" style="display:flex;align-items:flex-start;justify-content:center;padding-top:20px" @click.self="tryClose">
      <div class="glass p-4 w-full max-w-md mx-3 space-y-4 rounded-2xl max-h-[92vh] overflow-y-auto" @click.stop>
        ...
        <div class="space-y-2">
          ...
          <button v-if="entries.length" @click="printReport" class="btn-o w-full py-3.5 text-sm font-bold">
...
function printReport() {
  ...
  const svgHtml = `<svg viewBox="0 0 320 215" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:400px;display:block;margin:12px auto;background:#0e1a30;border-radius:12px;padding:8px">${svgContent}</svg>`
  ...
  if (!w) { toast('تم حظر النوافذ المنبثقة — اسمح بها للطباعة'); return }
  w.document.write(`<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>تقرير طب الأسنان</title>${printStyles}</head><body>${html}<div style="text-align:center;font-size:9px;color:#888;padding:10px">نظام طب الأسنان الرقمي — ${today}</div></body></html>`)
  w.document.close()
