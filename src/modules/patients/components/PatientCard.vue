    ...
    <div v-if="showingDetail" class="space-y-2 pt-2 border-t border-white/10">
      ...
      <div class="glass-sm p-2.5 rounded-xl" style="background:rgba(59,130,246,.04);border-color:rgba(59,130,246,.13)">
        ...
        <div v-if="xrayCount > 0" class="flex flex-wrap gap-1.5 mb-2">
          <div v-for="(img, i) in xrayList" :key="i" class="relative cursor-pointer" @click="$emit('open-xray', patient.name, i)">
            <img :src="xraySrc(img)" :alt="'أشعة'" class="w-[52px] h-[52px] object-cover rounded-lg border border-white/10" loading="lazy">
          </div>
        </div>
      ...
      <div v-if="patient.entries.length > 30" class="text-center text-[10px] opacity-40 py-2">
        +{{ patient.entries.length - 30 }} سجل إضافي
      </div>
    </div>
  </div>
</template>
...
import { uploadXrayImage } from '@/services/image.service'
...
async function onXrayUpload(e) {
  ...
  for (const file of files) {
    try {
      const key = await uploadXrayImage(file, name, uid)
      xrays.push(key)
    } catch (err) {
