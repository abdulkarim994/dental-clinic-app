...
export const usePatientsStore = defineStore('patients', () => {
  ...
  const patientMap = computed(() => {
    ...
    const cacheKey = `${recs.length}:${pros.length}:${dbts.length}:${(recs[0] || {})._mod || ''}:${(dbts[0] || {})._mod || ''}`
    ...
    all.forEach(r => {
      ...
      const enriched = { ...r, _s: r._t === 'p' ? 'p' : 'r' }
  ...
  function getPatientPhoto(name) {
    return getPatientPhotoFromStorage(name)
  }

  function savePatientPhoto(name, dataUrl) {
    savePatientPhotoToStorage(name, dataUrl)
  }
